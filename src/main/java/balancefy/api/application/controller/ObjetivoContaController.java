...
public class ObjetivoContaController {
    ...
    @GetMapping ResponseEntity<List<ObjetivoContaResponseDto>> getList(@RequestHeader(value = "Authorization") String token){
        ...
        return ResponseEntity.status(200).body(objetivoService.getList(accountId, 0));
    }

    @GetMapping("/done")
    ResponseEntity<List<ObjetivoContaResponseDto>> getDoneList(@RequestHeader(value = "Authorization") String token){
        Integer accountId = tokenService.getIdUsuario(token.replace("Bearer", ""));
        return ResponseEntity.status(200).body(objetivoService.getList(accountId, 1));
    }
